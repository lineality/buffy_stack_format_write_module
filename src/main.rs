...
pub enum FormatArg<'a> {
    ...
    CharStyled(char, Style),
    StrStyled(&'a str, Style),
    U8Styled(u8, Style),
    U64Styled(u64, Style),
    UsizeStyled(usize, Style),
    U8HexStyled(u8, Style),
}
...
/// Converts Style to ANSI escape sequences in provided stack buffer
///
/// ## Memory: ZERO HEAP
/// Concatenates ANSI codes directly into buffer.
fn style_to_ansi<'a>(style: Style, buf: &'a mut [u8]) -> Option<&'a str> {
    let mut pos = 0;
...
///
/// // With styling
/// buffy_print("Status: {}", &[FormatArg::StrStyled("OK", Style::bold_red())])?;
///
/// // With alignment
        ...
        if let Some(brace_pos) = template[pos..].find('{') {
            ...
            if let Some(close_pos) = template[absolute_brace..].find('}') {
                ...
                let (value_str, has_style, style) = match &args[arg_index] {
                    ...
                    FormatArg::Bool(b) => {
                        (if *b { "true" } else { "false" }, false, Style::default())
                    }
                    FormatArg::Char(c) => {
                ...
                if has_style {
                    let ansi = style_to_ansi(style, &mut style_buf).ok_or_else(|| {
                        io::Error::new(io::ErrorKind::Other, "Style conversion failed")
                    })?;
...
pub fn buffy_write<W: Write>(writer: &mut W, template: &str, args: &[FormatArg]) -> io::Result<()> {
    const MAX_ARGS: usize = 8;
